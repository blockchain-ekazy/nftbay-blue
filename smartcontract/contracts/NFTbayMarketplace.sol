...
contract NFTbayMarketplace is AccessControl, ReentrancyGuard {
    ...
    {
        NFTbayERC721.mint(_tokenURI, msg.sender);
    }
    ...
    function createOrder(uint256 _tokenId, uint256 _price) external {
        ...
        NFTbayERC721.transferFrom(msg.sender, address(this), _tokenId); // transfer token to contract
    ...
    function removeOrder(uint256 _orderId) external isOrderOwner(_orderId) {
        ...
        NFTbayERC721.transferFrom(address(this), _order.from, _order.tokenId); // transfer token to owner
    ...
    function _trade(TradeInnovice memory _tradeInnovice) private {
        ...
        bool isSecondarySale_ = NFTbayERC721.isSecondarySale(
            _tradeInnovice.tokenId
        );
        ...
        if (!isSecondarySale_) {
            NFTbayERC721.setSecondarySale(_tradeInnovice.tokenId);
        } else {
            (address receiver, uint256 royaltyAmount) = NFTbayERC721
                .royaltyInfo(_tradeInnovice.tokenId, sellerAmount);
